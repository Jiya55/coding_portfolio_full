...
'''def make_group(name):
    group= name.get()
    grp_mem= [mem.strip(' ') for mem in group.split(',')]
    for member in grp_mem:
        print(grp_mem)
...
def group():
    group_root= Tk()
    group_root.title()
    fg= Frame(group_root, height=400, width=300, bg='white')
    fg.propagate(0)
    fg.pack()
    grp_members= Entry(fg, width=20, bg='white')
    grp_members.place(x=170, y=10)
    name_lbl= Label(fg,text='Name of group members: ', bg='white')
    name_lbl.place(x=10,y=10)
    
    submit_btn = Button(fg, text='Submit', width=5, height=1, command=lambda:make_group(grp_members))
    submit_btn.place(x=200, y=350)
group()
'''
